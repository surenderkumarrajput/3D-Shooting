...
public class shield : MonoBehaviour {
	...
	void Update () 
	{
		...
		if (isguarding != lastguarding) 
		{
			StartCoroutine (doguard ());
		}
	...
	IEnumerator doguard()
	{
		if (isguarding) 
		{
			if (!isreloading)
			{
				myanimation.Play(guardAnim.name);
				yield return new WaitForSeconds (GetComponent<Animation> () [guardAnim.name].length);
			}
		...
		else 
		{
			if (!isreloading)
			{
				myanimation.Play(unguardAnim.name);
				yield return new WaitForSeconds (GetComponent<Animation> () [unguardAnim.name].length);
			}
